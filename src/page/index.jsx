import React from "react";
import "./index.css";

export default function Main() {
  return (
    <div className="main-container">
      <div className="dashboard-before-login">
        <div className="group" />
        <div className="flex-row-e">
          <div className="group-1">
            <div className="group-2">
              <span className="welcome-to-gynexa">
                {" "}
                Chào mừng bạn đến với Gynexa{" "}
              </span>
              <span className="body-awareness">
                Bắt nhịp cơ thể, hiểu thấu chính mình
              </span>
            </div>
            <span className="gynexa-introduction">
              GYNEXA là trung tâm y tế chuyên khoa hàng đầu Việt Nam trong lĩnh
              vực chăm sóc sức khỏe sinh sản và giới tính. Được thành lập năm
              2008 với tầm nhìn trở thành trung tâm y tế đạt chuẩn quốc tế,
              chúng tôi đã không ngừng đầu tư vào công nghệ y tế tiên tiến và
              đào tạo đội ngũ chuyên gia đẳng cấp thế giới.
              <br />
            </span>
            <div className="flex-row-e-3">
              <div className="link-button">
                <div className="vector" />
              </div>
              <div className="button-with-icon">
                <span className="read-more">Xem thêm</span>
                <div className="vector-4" />
              </div>
            </div>
          </div>
          <div className="comment-icon" />
          <div className="group-5">
            <div className="rectangle" />
            <div className="group-6">
              <span className="gynexa-services">Dịch vụ của GYNEXA</span>
            </div>
            <div className="group-7">
              <div className="group-8">
                <div className="group-9">
                  <div className="medical">
                    <div className="medical-icon" />
                  </div>
                  <span className="online-consultation">Tư vấn trực tuyến</span>
                </div>
                <div className="group-a">
                  <div className="vector-b" />
                  <span className="sti-testing">Xét nghiệm STIs</span>
                </div>
                <div className="group-c">
                  <span className="faq">Hỏi đáp</span>
                </div>
                <div className="group-d" />
                <div className="rectangle-e" />
                <div className="rectangle-f" />
                <div className="group-10">
                  <div className="cycle-tracking" />
                  <span className="flex-column-b">Theo dõi chu kỳ</span>
                </div>
              </div>
              <div className="group-11">
                <div className="group-12" />
                <div className="group-13" />
              </div>
              <div className="personalized-healthcare">
                <span className="group-14">
                  Chăm sóc sức khỏe giới tính một cách cá nhân hóa & an toàn
                </span>
                <div className="flex-row-dab">
                  <div className="group-15">
                    <div className="cycle-tracking-16">
                      <span className="rectangle-17">Theo dõi chu kỳ</span>
                      <div className="group-18" />
                    </div>
                    <div className="sti-testing-19">
                      <span className="rectangle-1a">Xét nghiệm STIs</span>
                      <div className="flex-row-ef" />
                    </div>
                  </div>
                  <div className="group-1b">
                    <div className="ovulation-tracking">
                      <span className="text-d">
                        Theo dõi thời gian rụng trứng
                      </span>
                      <div className="rectangle-1c" />
                    </div>
                    <div className="group-1d">
                      <span className="reminder-drug">Nhắc nhở uống thuốc</span>
                      <div className="rectangle-1e" />
                    </div>
                  </div>
                  <div className="flex-row-dda">
                    <div className="group-1f">
                      <span className="gender-counseling">
                        Tư vấn giới tính 1 : 1
                      </span>
                      <div className="rectangle-20" />
                    </div>
                    <div className="group-21">
                      <span className="online-counseling">
                        Tư vấn trực tuyến
                      </span>
                      <div className="rectangle-22" />
                    </div>
                  </div>
                </div>
                <div className="group-23">
                  <span className="gynexa-platform">
                    Gynexa là nền tảng chăm sóc sức khỏe giới tính hiện đại,
                    hướng đến sự an toàn, chủ động và thấu hiểu cho từng cá
                    nhân.Gynexa cung cấp giải pháp toàn diện - từ tư vấn trực
                    tuyến, theo dõi chu kỳ sinh sản đến xét nghiệm STIs an toàn
                    và bảo mật
                  </span>
                  <span className="user-experience">
                    Lorem Người dùng có thể dễ dàng đặt lịch tư vấn với đội ngũ
                    chuyên gia giàu kinh nghiệm, gửi câu hỏi và theo dõi toàn bộ
                    hồ sơ sức khỏe của mình qua một nền tảng duy nhất - bảo mật,
                    thông minh và đầy sự thấu cảm.
                    <br />
                  </span>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="rectangle-24">
          <div className="rectangle-25" />
          <div className="rectangle-26">
            <div className="flex-row-c">
              <div className="rectangle-27">
                <span className="name">Tên</span>
              </div>
              <div className="rectangle-28">
                <span className="gender">Giới tính</span>
                <div className="vector-29" />
              </div>
            </div>
            <div className="flex-row-e-2a">
              <div className="rectangle-2b">
                <span className="email">Email</span>
              </div>
              <div className="rectangle-2c">
                <span className="phone">Số điện thoại</span>
              </div>
            </div>
            <div className="flex-row-ce">
              <div className="rectangle-2d">
                <span className="date">Ngày </span>
                <div className="vector-2e" />
              </div>
              <div className="rectangle-2f">
                <span className="time">Giờ</span>
                <div className="vector-30" />
              </div>
            </div>
            <div className="flex-row-edf">
              <div className="rectangle-31">
                <span className="doctor">Bác sĩ</span>
                <div className="vector-32" />
              </div>
              <div className="group-33" />
            </div>
            <div className="rectangle-34">
              <span className="message">Ghi chú</span>
            </div>
            <div className="rectangle-35">
              <span className="submit">XÁC NHẬN</span>
            </div>
          </div>
          <span className="schedule-consultation">
            Đặt lịch để được tư vấn miễn phí ngay
          </span>
          <span className="gender-healthcare">
            Chăm sóc sức khỏe giới tính bắt đầu từ sự thấu hiểu. Từ theo dõi chu
            kỳ kinh nguyệt đến đặt lịch tư vấn riêng tư, Gynexa đồng hành cùng
            bạn trên từng nhịp sống. Trải nghiệm nền tảng y tế thông minh, nơi
            xét nghiệm STIs được quản lý an toàn, kết quả bảo mật và hỗ trợ
            chuyên sâu từ đội ngũ tư vấn viên giàu kinh nghiệm. Tại đây, mỗi cơ
            thể đều được lắng nghe, mỗi thắc mắc đều được giải đáp, và mỗi hành
            trình sức khỏe đều mang tính cá nhân hóa
          </span>
        </div>
        <span className="our-doctors">Về đội ngũ y tế của Gynexa</span>
        <div className="flex-row-acf">
          <div className="doctors-card">
            <div className="rectangle-36" />
            <div className="group-37">
              <span className="doctor-name">Doctor’s Name</span>
              <span className="neurology">Neurology</span>
              <div className="rectangle-38" />
              <div className="vector-39" />
              <div className="vector-3a" />
              <div className="vector-3b" />
            </div>
            <div className="group-3c">
              <span className="view-profile">View Profile</span>
              <div className="rectangle-3d" />
            </div>
          </div>
          <div className="doctors-card-3e">
            <div className="rectangle-3f" />
            <div className="group-40">
              <span className="doctors-name">Doctor’s Name</span>
              <span className="neurology-41">Neurology</span>
              <div className="rectangle-42" />
              <div className="vector-43" />
              <div className="vector-44" />
              <div className="vector-45" />
            </div>
            <div className="group-46">
              <span className="view-profile-47">View Profile</span>
              <div className="rectangle-48" />
            </div>
          </div>
          <div className="doctors-card-49">
            <div className="rectangle-4a" />
            <div className="group-4b">
              <span className="doctors-name-4c">Doctor’s Name</span>
              <span className="neurology-4d">Neurology</span>
              <div className="rectangle-4e" />
              <div className="vector-4f" />
              <div className="vector-50" />
              <div className="vector-51" />
            </div>
            <div className="group-52">
              <span className="view-profile-53">View Profile</span>
              <div className="rectangle-54" />
            </div>
          </div>
        </div>
        <div className="group-55" />
        <div className="rectangle-56">
          <span className="better-information-health">
            Cập nhật tri thức – Dẫn lối sức khỏe
          </span>
          <span className="news">Blog</span>
          <div className="flex-row-b">
            <div className="rectangle-57">
              <div className="rectangle-58" />
              <span className="monday-september-author">
                Monday 05, September 2021 | By Author
              </span>
              <span className="article-title">
                This Article’s Title goes Here, <br />
                but not too long.
              </span>
              <span className="text-2c">68</span>
              <span className="text-2d">86</span>
              <div className="vector-59" />
              <div className="group-5a" />
            </div>
            <div className="rectangle-5b">
              <div className="rectangle-5c" />
              <span className="monday-september-author-5d">
                Monday 05, September 2021 | By Author
              </span>
              <span className="article-title-5e">
                This Article’s Title goes Here, <br />
                but not too long.
              </span>
              <span className="text-30">68</span>
              <span className="text-31">86</span>
              <div className="vector-5f" />
              <div className="group-60" />
            </div>
          </div>
          <div className="flex-row">
            <div className="rectangle-61">
              <div className="rectangle-62" />
              <span className="author-info">
                Monday 05, September 2021 | By Author
              </span>
              <span className="article-title-63">
                This Article’s Title goes Here, <br />
                but not too long.
              </span>
              <span className="number">68</span>
              <span className="number-64">86</span>
              <div className="vector-65" />
              <div className="group-66" />
            </div>
            <div className="rectangle-67">
              <div className="rectangle-68" />
              <span className="author-info-69">
                Monday 05, September 2021 | By Author
              </span>
              <span className="article-title-6a">
                This Article’s Title goes Here, <br />
                but not too long.
              </span>
              <span className="number-6b">68</span>
              <span className="number-6c">86</span>
              <div className="vector-6d" />
              <div className="group-6e" />
            </div>
          </div>
        </div>
        <div className="group-6f" />
        <span className="get-in-touch">
          Đừng giữ thắc mắc một mình – hỏi ngay nhé!
        </span>
        <span className="contact">Liên hệ</span>
        <div className="flex-row-de">
          <div className="rectangle-70">
            <div className="group-71" />
            <span className="emergency">đường dây nóng</span>
            <span className="phone-number">(237) 681-812-255</span>
            <span className="phone-number-72">(237) 666-331-894</span>
          </div>
          <div className="rectangle-73">
            <div className="group-74" />
            <span className="location">địa chỉ</span>
            <span className="address">0123 Some place</span>
            <span className="country">9876 Some country</span>
          </div>
          <div className="rectangle-75">
            <div className="group-76" />
            <span className="email-77">Email</span>
            <span className="email-address">fildineeesoe@gmil.com</span>
            <span className="email-address-78">myebstudios@gmail.com</span>
          </div>
          <div className="rectangle-79">
            <div className="group-7a" />
            <span className="working-hours">giờ làm việc</span>
            <span className="working-hours-7b">T2-T7 09:00-20:00</span>
            <span className="emergency-hours">Chủ Nhật 24h</span>
          </div>
        </div>
        <div className="footer">
          <div className="vector-7c" />
          <div className="rectangle-7d" />
          <div className="group-7e">
            <span className="medical-7f">GYNEXA</span>
            <span className="medical-info">
              Bắt nhịp cơ thể. <br />
              Hiểu thấu chính mình
            </span>
          </div>
          <div className="group-80">
            <span className="important-links">Important Links</span>
            <div className="group-81">
              <span className="appointment">Appointment</span>
              <span className="doctors">Doctors</span>
              <span className="services">Services</span>
              <span className="about-us">About Us</span>
            </div>
          </div>
          <div className="group-82">
            <span className="contact-us">Contact Us</span>
            <div className="group-83">
              <span className="call-info">Call: (237) 681-812-255</span>
              <span className="email-info">Email: fildineesoe@gmail.com</span>
              <span className="address-info">Address: 0123 Some place</span>
              <span className="country-info">Some country</span>
            </div>
          </div>
          <div className="group-84">
            <span className="newsletter">Newsletter</span>
            <div className="group-85">
              <div className="rectangle-86" />
              <div className="vector-87" />
              <span className="email-input">Enter your email address</span>
            </div>
          </div>
          <div className="vector-88" />
          <div className="vector-89" />
          <div className="vector-8a" />
          <span className="copyright-info">
            © 2021 Hospital’s name All Rights Reserved by PNTEC-LTD
          </span>
        </div>
        <div className="flex-row-cb">
          <div className="rectangle-8b" />
          <div className="frame">
            <div className="link-item">
              <span className="login">Đăng nhập</span>
            </div>
            <div className="register">
              <span className="frame-8c">Đăng kí</span>
            </div>
          </div>
          <div className="streamline-health-care">
            <div className="gynexa" />
            <span className="home">Gynexa</span>
          </div>
          <span className="services-8d">Trang chủ</span>
          <span className="book-appointment">Dịch vụ</span>
          <span className="blog">Đặt lịch</span>
          <span className="flex-row-caa">Blog</span>
        </div>
        <div className="frame-8e">
          <div className="frame-8f" />
          <div className="frame-90">
            <div className="group-91" />
            <div className="rectangle-92" />
            <div className="frame-93">
              <div className="frame-94">
                <span className="service-info">
                  Dịch vụ chăm sóc sức khỏe giới tính Gynexa{" "}
                </span>
                <span className="free-consultation">Tư vấn miễn phí</span>
              </div>
              <div className="frame-95">
                <div className="frame-96" />
                <div className="frame-97">
                  <span className="online-consultants">
                    +18 tư vấn viên đang trực tuyến
                  </span>
                </div>
              </div>
            </div>
            <div className="ellipse" />
          </div>
          <div className="frame-98">
            <div className="frame-99">
              <div className="magnifying-glass" />
              <span className="search-input">
                Tìm kiếm thông tin, dịch vụ, ...
              </span>
            </div>
            <div className="frame-9a">
              <div className="vector-9b" />
              <div className="link-item-9c">
                <span className="search-button">Tìm kiếm</span>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
